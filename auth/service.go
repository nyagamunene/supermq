...
func New(keys KeyRepository, pats PATSRepository, hasher Hasher, idp magistrala.IDProvider, tokenizer Tokenizer, policyEvaluator policies.Evaluator, policyService policies.Service, loginDuration, refreshDuration, invitationDuration time.Duration) Service {
	return &service{
		tokenizer:          tokenizer,
		keys:               keys,
		pats:               pats,
...
func (svc service) RetrievePAT(ctx context.Context, session authn.Session, patID string) (PAT, error) {
	pat, err := svc.pats.Retrieve(ctx, session.UserID, patID)
	if err != nil {
