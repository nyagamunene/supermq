...
func (ms *metricsMiddleware) ViewChannel(ctx context.Context, session authn.Session, id string, getRoles bool) (channels.Channel, error) {
	defer func(begin time.Time) {
		ms.counter.With("method", "view_channel").Add(1)
		ms.latency.With("method", "view_channel").Observe(time.Since(begin).Seconds())
	}(time.Now())
	return ms.svc.ViewChannel(ctx, session, id, getRoles)
}
