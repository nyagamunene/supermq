...
func (am *authorizationMiddleware) ViewChannel(ctx context.Context, session authn.Session, id string, getRoles bool) (channels.Channel, error) {
	if session.Type == authn.PersonalAccessToken {
		if err := am.authz.AuthorizePAT(ctx, smqauthz.PatReq{
			UserID:           session.UserID,
			PatID:            session.PatID,
	...
	return am.svc.ViewChannel(ctx, session, id, getRoles)
}
