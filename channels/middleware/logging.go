...
func (lm *loggingMiddleware) ViewChannel(ctx context.Context, session authn.Session, id string, getRoles bool) (c channels.Channel, err error) {
	defer func(begin time.Time) {
		args := []any{
			slog.String("duration", time.Since(begin).String()),
			slog.String("domain_id", session.DomainID),
	...
	return lm.svc.ViewChannel(ctx, session, id, getRoles)
}
