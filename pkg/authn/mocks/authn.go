...
// AuthenticatePAT provides a mock function with given fields: ctx, token
func (_m *Authentication) AuthenticatePAT(ctx context.Context, token string) (authn.Session, error) {
	ret := _m.Called(ctx, token)

	if len(ret) == 0 {
		panic("no return value specified for AuthenticatePAT")
	}
